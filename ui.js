 ...
 * Changelog:
 * v2.18.0 - 2025-08-29
 * - Reworked `populateCategoryFilters` to render a nested, collapsible UI.
 * - Function now accepts a Map of category-to-subcategory relationships.
 *
 * v2.17.0 - 2025-08-29
 * - Added `populateCategoryFilters` function to dynamically render category filters.
 *
 * v2.16.0 - 2025-08-28
...
export function populateCategoryFilters(categoryMap) {
    const container = document.getElementById('category-filter-container');
    if (!container || categoryMap.size === 0) return;

    let html = '<h4>Categories</h4>';
    const sortedCategories = [...categoryMap.keys()].sort();

    for (const category of sortedCategories) {
        const subcategories = [...categoryMap.get(category)].sort();
        ...
        html += `
            <label class="category-label">
                ...
                ${category}
                ${subcategories.length > 0 ? '<span class="arrow">▶</span>' : ''}
            </label>
        `;
        ...
        if (subcategories.length > 0) {
            ...
            subcategories.forEach(subcat => {
                html += `
                    <label>
                        <input type="checkbox" class="subcategory-checkbox" name="subcategory" value="${subcat}">
                        ${subcat}
                    </label>
                `;
            });
...
export async function showDetailModal(record) {
 ...
 const addToPlanBtn = document.getElementById('modal-add-to-plan-btn');
        if (addToPlanBtn) {
            addToPlanBtn.textContent = lockedItemInfo ?

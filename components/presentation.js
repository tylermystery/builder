...
async function renderCompactCard(item) {
    ...
    const summaryEmoji = getItemSummaryEmoji(recordId);
    const reactionCount = getItemReactionCount(recordId);
    const rankingTooltip = reactionRankings.getItemRankingTooltip(recordId);
    ...
    const cachedComments = componentComments.getCache().get(commentCacheKey);
...
async function renderCompactGroupCard(group) {
    ...
    for (const gId of groupItems) {
        const memberComments = componentComments.getCache().get(`item:${gId}`);
...
function buildRSBPanelDOM(recordId, isModal) {
    ...
    const reactionCount = getItemReactionCount(recordId);
    const commentCacheKey = `item:${recordId}`;
    const comments = componentComments.getCache().get(commentCacheKey) || [];
...
function buildRSBCommentsContent(container, recordId) {
    ...
    const comments = componentComments.getCache().get(cacheKey) || [];
...
async function submitRSBComment(recordId, container) {
    ...
    try {
        ...
        componentComments.getCache().set(cacheKey, freshComments);
...
async function setItemAsGoal(recordId) {
    ...
    planFocus.triggerPlanFocusGeneration();
 ...
 */
// --- Plan Focus moved to presentation/planFocus.js ---
// Delegate calls: planFocus.renderGoalChips(), planFocus.initializeHandlers(), planFocus.triggerPlanFocusGeneration()

 ...
 ui.updateFavoritesCarousel();
    ...
    paletteSortable = new Sortable(paletteContainer, {
        ...
        onEnd: function(evt) {
            ...
            // An item was moved from the timeline back 
 to the palette
            if (evt.from.id !== evt.to.id) {
                const recordId = evt.item.dataset.recordId;
            ...
            renderItinerary(); // Re-render both columns to be 
 safe
            ui.updateFavoritesCarousel();
            ui.updateTotalCost();
 triggerSave();
...
export function showItineraryModal() {
    ...
    renderItinerary();
 // Render after header to ensure date is set
...
async function createItineraryItem(record, itemInfo, type, timeInfo = null) {
 ...
 // --- Add event listeners for removing items ---
    itemElement.querySelector('.remove-btn').addEventListener('click', (e) => {
        ...
        renderItinerary(); // Re-render everything 
 after removal
        ui.updateAllUI();
        triggerSave();
    });

...
The application is built on a modular architecture to ensure maintainability and scalability.
Each JavaScript file has a distinct responsibility, separating concerns like data management, API communication, user interaction, and UI rendering.
...
* **imageManager.js**: A centralized manager that queues, debounces, and batches image requests to the Cloudinary API to prevent rate-limiting and improve performance.
...
* **components/itinerary.js**: Manages the drag-and-drop itinerary builder modal, allowing users to organize their selected items.
* **components/presentation.js**: Manages the full-screen "Presentation View" for an immersive, slideshow-style browsing experience of selected item lists.

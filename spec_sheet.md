...
### Changelog
* **v1.2.5 (2025-09-11):** Polished the checkout flow with improved user feedback and autofill. Corrected the search logic to be more intuitive. Fixed various UI bugs, including the event plan sidebar actions and the chat user naming process.
...
### Search & Filtering
A set of controls allows users to refine and organize the catalog view in an intuitive, top-down manner.
* **Category Buttons**: Buttons at the top of the panel allow users to filter by category. Selecting a category will recursively display all bookable items nested underneath it.
* **Smart Search**: A search bar allows users to filter the catalog by item name, description, categories, and tags. This search is applied *after* category and attribute filters, allowing users to search within a specific subset of items. Results are prioritized to show name matches first.

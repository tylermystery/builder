// FILE: netlify/functions/update-user-prefs.js (REPLACE ENTIRE FILE WITH ADDED LOGGING)
...
async function handleToggleLike(userId, itemId) {
    ...
    // Mask PAT for security, showing only first/last few chars
    ...
    if (!AIRTABLE_PAT) console.error("[func-combo] CRITICAL: AIRTABLE_PAT environment variable is missing!");
    if (!BASE_ID) console.error("[func-combo] CRITICAL: BASE_ID environment variable is missing!");
    ...
    let itemRes; // Declare outside try block
    try { // Wrap fetch in try/catch to log specific fetch errors
    ...
    if (!itemRes.ok) {
        const errorStatus = itemRes.status; // Get status code
        ...
        throw new Error(`Failed to fetch item ${itemId} from Airtable. Status: ${errorStatus}`); // Include status
    }
    ...
    console.log(`[func-combo] handleToggleLike: Patch payload:`, JSON.stringify(payload)); // Log payload

    let patchRes; // Declare outside try block
    try { // Wrap fetch in try/catch
    ...
    if (!patchRes.ok) {
        const errorStatus = patchRes.status; // Get status code
        ...
        throw new Error(`Failed to update likes for item ${itemId} in Airtable. Status: ${errorStatus}`); // Include status
    }
...
async function handleUpdatePrefs(userId, phone, frequency) {
    ...
    console.log('[func-combo] handleUpdatePrefs: Patch payload:', JSON.stringify(payload)); // Log payload

    const patchUrl = `https://api.airtable.com/v0/${BASE_ID}/${USERS_TABLE}`; // Ensure USERS_TABLE is correct
  ...
  try {
    ...
    if (action === 'toggle-like') {
      ...
      const { phone, frequency } = body;
       if (!['Real-Time', 'None'].includes(frequency)) { // Basic validation
    ...
    console.error(`[func-combo] Handler Error processing action '${body?.action || 'unknown'}':`, error); // Log which action failed
    return {
      statusCode: 500,
      headers: { 'Content-Type': 'application/json' },
